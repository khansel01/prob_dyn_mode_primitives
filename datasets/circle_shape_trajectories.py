"""
This file generates circle shapes trajectories
"""
...
class CircleShapes(BaseDataset):
    def __init__(self, t_steps: jnp.ndarray, **kwargs):
        """  Generate batches of circle shapes
        :param t_steps: Time steps as a numpy nd.array of size T.
        :param kwargs:
                sigma: Describes the variability of the circle shapes as a float value.
    ...
    def transform(self) -> jnp.ndarray:
        """ Generate a batch of circle shapes
    ...
    def _fun(self, samples: jnp.ndarray, sigmas: jnp.ndarray) -> jnp.ndarray:
        for s in range(samples.shape[0]):
            ...
            samples = i_update(samples, indx[s, 1, :],
                               (sigmas[s, 3] + 0.75) * jnp.sin(2 * jnp.pi * self.t_steps + sigmas[s, 4]) + sigmas[s, 5])
            samples = i_update(samples, indx[s, 2, :],
                               (sigmas[s, 6] + 0.75) * jnp.sin(2 * jnp.pi * self.t_steps + sigmas[s, 7]) + sigmas[s, 8])
        return samples
