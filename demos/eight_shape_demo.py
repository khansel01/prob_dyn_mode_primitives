...
# --------------------------------------------------------------------------------------------------------------------
# ----------------------------------- Example of GP-DMD on an EightShape Data Set ------------------------------------
# --------------------------------------------------------------------------------------------------------------------


def eight_shape_example(args: argparse.Namespace) -> Tuple:
    """
    ...
    eight_shapes = EightShapes(t_steps, s_size=args.number_demonstrations, sigma=args.variability,
                               prng_handler=prng_handler)

    y = jnp.concatenate(eight_shapes.transform, axis=1).T

    standarizer = Whitening(y.reshape(1, -1, order='F'))

    y = jnp.float64(standarizer().T.reshape(*y.shape, order='F'))

    y = y.reshape(eight_shapes.transform.transpose((0, 2, 1)).shape)
    ...
    _kwargs = {'suptitle': r'Eight Shape Latent Space', 'subtitle': '$\mathrm{Latent~dim}$',
...
if __name__ == '__main__':
    ...
    mse, std = eight_shape_example(args)
