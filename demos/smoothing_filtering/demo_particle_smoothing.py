...
from utilities.inference_lib import ParticleSmoother
...
def demo():
    """
    This demo shows an example of particle smoothing on two simplex examples
    :return: None
    """
    ...
    # Filtering & Smoothing
    sir = ParticleSmoother(pi_x0, pi_x, p_x, p_y, initial[0], (sigma_x, sigma_y),
                           num_particles=num_particles, iterations=len(t_avail), prng_handler=prng_handler)
    sir.filtering(_Y)
    sir.smoothing()
    # End

    particles =  sir.particles
    particle_smoothed = jnp.array(sir.smooth_particle).T

    # ----------------------------------------------- Visualization ---------------------------------------------------

    fig = plt.figure()
    grid = fig.add_gridspec(2, 2)
    plt.suptitle(f"Bootsstrap Prticle Filter in combination with \n Backward-Simulation Particle Smoother")
    ...
    ax10.plot(t_avail, particle_smoothed[0], 'b', label="Smoothed")
    plt.legend()
    ...
    ax20.plot(t_avail, particle_smoothed[0], 'b', label="Smoothed")
