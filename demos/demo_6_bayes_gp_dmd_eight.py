...
def demo(variables=None):
    """
    This demo shows an example of Bayesian GP DMD on the eight shape dataset.
    :return: None
    """
    ...
    # x = y @ _v[:, :m]
    x = jnp.ones((t, m)) * jnp.cos(2*jnp.pi * t_steps.reshape(-1, 1)) + \
         jax.random.uniform(prng_handler.get_keys(1)[0], (1, m), minval=-1, maxval=1)
    x = index_update(x, index[:, :_v.shape[1]], y @ _v)
